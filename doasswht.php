...
$whtratex=$_POST["whtrate"];



$startdate = isset($_POST['startdate']) ? $_POST['startdate'] : null;
$enddate = isset($_POST['enddate']) ? $_POST['enddate'] : null;
$whtpaid=number_format(str_replace( ',', '',mysqli_real_escape_string($conn,str_replace( ',', '',preg_replace('/\s+/', '', isset($_POST['whtamtpaid']) ? $_POST['whtamtpaid'] : 0.00)))),2);

$whttnx=isset($_POST['whttnx']) ? $_POST['whttnx'] : 'no';
$whtpen=isset($_POST['penintwht']) ? $_POST['penintwht'] : 'off';
...
<body>
   <div class="container-fluid">
           <div id="verify">
          <form name="datacap" action="dosubmitasswht" method="post" target="_blank">
...
//				 vat comes here
				...
				//  $totalvat=$amountray-$inputtax-$vatpaid;
                 ...
                 // WHT COMES HERE
                $totwhttnx=($amountray+$whtpaidamt)/$whtrate;
                $diswht=$totwhttnx*$whtrate;
				 $whtout=$totwhttnx*$whtrate;
				 $whtdue=$whtout-$whtpaidamt;
                 $diswhtpen=$whtdue*($totpenint/100);
                  $distotalwht=$whtdue+$diswhtpen;
                 // WHT ENDS HERE
                ...
                <span> '.number_format($totwhttnx,2).'</span>
                    </div>
					
					<div class="form-group">
                    <label for="coyname">Withholding Tax @ '.(int)$whtratex.'%: </label>
                    <input type="hidden" class="form-control" id="pen" placeholder="Interest & Penalty" hidden="hidden">
                <span> '.number_format($diswht,2).'</span>
                    </div>
                ...
                <span> '.number_format($whtpaidamt,2).'</span>
                    </div>
                    
                    <div class="form-group">
                    <label for="coyname">WHT Outstanding</label>
                    <input type="hidden" class="form-control" id="amt" placeholder="Total Amt" hidden="hidden">
                <span> '.number_format($whtdue,2).'</span>
                    </div>';
						 }
				 ...
				 if ($whtpen=="on" ){
                    ...
                    <input type="hidden" class="form-control" id="pen" placeholder="Interest & Penalty" hidden="hidden">
                <span> '.number_format($diswhtpen,2).'</span>
                    </div>
                    
                    <div class="form-group" id="fin">
                    <label for="coyname">Total WHT Amount Payable:</label>
                    <input type="hidden" class="form-control" id="amt" placeholder="Total Amt" hidden="hidden">
                <span> '.number_format($distotalwht,2).'</span>
                    </div>'; 
               ...
               <input type="hidden" value="<?php echo $whtratex;?>"  id="whtrate" name="whtrate">
               <input type="hidden" value="<?php echo $whtpaidamt;?>"  id="whtpaid" name="whtpaid">
               <input type="hidden" value="<?php echo $whttnx;?>"  id="whttnx" name="whttnx">
               <input type="hidden" value="<?php echo $whtpen;?>"  id="whtpen" name="whtpen">
   ...
   <script src="Ajaxsubmit3.js"></script>
