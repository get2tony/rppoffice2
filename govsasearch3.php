...
$term="";
$table="adminasreg";
 $showterm="All Audit Asmt";
 $tableshow="Audit";

$yoa = isset($_REQUEST['catb']) ? $_REQUEST['catb'] : date('Y');
...
$sql="SELECT * FROM $table WHERE capdate LIKE '%".$yoa."%' && basis LIKE 'Audit' && `approval` LIKE 'approved' ORDER BY capdate ASC";
}
if ($term=="all"){
...
$sql="SELECT * FROM $table WHERE capdate LIKE '%".$yoa."%' && basis LIKE 'Audit' && `approval` LIKE 'approved'ORDER BY capdate ASC";
 ...
 if($term=="cit"){ 
      $showterm="CIT Raised";
$sql="SELECT * FROM `$table` WHERE `taxtype` like 'CIT' && capdate like '%".$yoa."' && basis LIKE 'Audit' && `approval` LIKE 'approved' ORDER BY capdate ASC";
 }
if($term=="edt"){ 
      $showterm="EDT Raised";
$sql="SELECT * FROM `$table` WHERE `taxtype` like 'EDT' && capdate like '%".$yoa."' && basis LIKE 'Audit' && `approval` LIKE 'approved' ORDER BY capdate ASC";
 }

if($term=="vat"){ 
      $showterm="VAT Raised";
$sql="SELECT * FROM `$table` WHERE `taxtype` like 'VAT' && capdate like '%".$yoa."' && basis LIKE 'Audit' && `approval` LIKE 'approved' ORDER BY capdate ASC";
 }
if($term=="wht"){ 
      $showterm="WHT Raised";
$sql="SELECT * FROM `$table` WHERE `taxtype` like 'WHT' && capdate like '%".$yoa."' && basis LIKE 'Audit' && `approval` LIKE 'approved' ORDER BY capdate ASC";
 }
...
<body>
   ...
   <div id="verify">
    <form name="search" action="govsasearch3 "  method="post">
                   ...
                   <select name=cata onchange="this.form.submit();">
                        ...
                        if ($term!='all') {
                            ...
                            echo '<option value="all">All Audit Asmt</option>';
                        }
                       ...
                       <option value="all">All Audit Asmt</option>
                   </select>
              
               &nbsp;&nbsp;
                    <label for="coyname">Raised in:</label>
                    <input type="text" name="catb" value="<?php echo $yoa ?>" />
