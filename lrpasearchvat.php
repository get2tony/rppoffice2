...
$tableshow = "Late Submission Penalty";

if ($table=='lrpcurrent'){
    $table = 'lrpcurrent';
    $sql = "SELECT * FROM $table WHERE capdate LIKE '%" . $term . "' && tax LIKE 'VAT' ORDER BY sno DESC";
}
...
<body>
   ...
   <div id="verify">
    <form name="search" action="lrpasearchvat"  method="post">
        ...
        <option value="<?php echo $table ?>"><?php
			...
			?> </option>
                       ...
                       <option value="lrpcurrent">LSP Current</option>
					   <option value="lrpback_year">LSP Back Year</option>
            ...
            echo '<tr>
               ...
               <input type="hidden" value="'.$end.'"  id="end" name="end"/>
                    <button  id="foo" type="Submit" class ="btn btn-primary" data-toggle="tooltip" title="View Record! ">
                        ...
                        <a data-toggle="tooltip" title="Edit Record!"target="dframe" href="editlrpvat?user='.$suser.'&usersno='.$usersno.'&tin='.$row['tinno'].'&yrend='.$row['yearend'].'&tab='.$table.'&sno='.$row['sno'].'"><span class="glyphicon glyphicon-edit" alt="edit Records"></span></a>
                        <a  data-toggle="tooltip" title="Delete Record!"target="dframe" href="deletelrpvat?user='.$suser.'&usersno='.$usersno.'&tin='.$row['tinno'].'&yrend='.$row['yearend'].'&tab='.$table.'&sno='.$row['sno'].'"><span class="glyphicon glyphicon-trash" alt="Delete Records"></span></a>
                   </form>';
                    }else {

                     echo ' <form action="viewvatlsp" method="post" target="_blank">
               ...
               <input type="hidden" value="'.$end.'"  id="end" name="end"/>
                    <button  type="Submit" class ="btn btn-primary" data-toggle="tooltip" title="View Record! ">
                        <span class="glyphicon glyphicon-print" alt="View Records"></span> </button></form>
                     ';
                    }
