...
if ($table!='both_self') {
    $sql = " SELECT * FROM $table WHERE (niltax like 'Yes' || cit<1 ) && capdate like '%" . date('Y') . "%' ORDER BY capdate DESC";
}else {
...
<head>
    <meta charset="UTF-8">
    <title>Report- <?php echo $showterm ?> in <?php echo $table ?> Register Filed in <?php echo date('Y') ?> </title>
...
<body>
   ...
   <div id="verify">
    <form name="search" action="nilsearchyoa2 "  method="post">
       
                
                    <label for="">Search For NIL Filers :</label>
                  <input name="cata" value="All Filers" />
               ...
               &nbsp;&nbsp;
                ...
                <font  class="pull-center" color="red"><?php echo $count ?> Record(s) Found of <?php echo $showterm ?> in <?php echo date('Y') ?>  </font>
                </form>
       </div>
       </div>
                <hr>
