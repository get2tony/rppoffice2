...
$term = isset($_REQUEST['cata']) ? $_REQUEST['cata'] : "";
...
$table="current";
$sql="SELECT * FROM $table WHERE yoa='".date('Y')."' && capdate LIKE '%".date('Y')."%' ORDER BY asmtno ASC";
}
if ($term=="all"){
...
$sql="SELECT * FROM $table WHERE yoa='".date('Y')."'  && capdate LIKE '%".date('Y')."%' ORDER BY asmtno ASC";
 ...
 if($term=="niltax"){ 
      $showterm="Nil Filers";
$sql="SELECT * FROM `$table` WHERE `niltax` like 'yes' && yoa='".date('Y')."' && capdate like '%".date('Y')."' ORDER BY asmtno ASC";
 }

if($term=="mintax"){ 
     $showterm="Minimum Tax Filers";
$sql="SELECT * FROM `$table` WHERE `mintax` like 'yes' && capdate like '%".date('Y')."' ORDER BY asmtno ASC ";
}

 if($term=="top"){ 
      $showterm="Top Taxpayers";
$sql="SELECT * FROM `$table` WHERE capdate like '%".date('Y')."'  ORDER BY `cit` DESC";
 }

if($term=="low"){ 
     $showterm="Low Taxpayers";
$sql="SELECT * FROM `$table` WHERE capdate like '%".date('Y')."'  ORDER BY `cit` ASC";
}
...
<head>
    <meta charset="UTF-8">
    <title>Report- <?php echo $showterm ?> From <?php echo $table ?> Register</title>
...
<body>
   ...
   <div id="verify">
    <form name="search" action="sasearch "  method="post">
       
                
                    <label for="">Search For :</label>
                   <select name="cata" onchange="this.form.submit();">
                      
                       <option value="<?php echo $term ?>"><?php echo ucfirst($term) ?> Taxpayers</option>
                       <option value="all">All Taxpayers</option>
                       <option value="top">Top Paying Taxpayers</option>
                       <option value="low">Low Paying Taxpayers</option>
                       <option value="niltax">Nil Filers</option>
                       <option value="mintax">Minimum Tax Filers</option>
