...
if($assprofit===null || $assprofit==0){
	$assprofit=$amount;
}
if ($tprofit===null || $tprofit==0){
...
if($taxtype=="CIT"){
 ...
 $rate=round(getSettings2($serial,'citrate',$conn)).'%';
}
if($taxtype=="WHT" || $taxtype=="CGT"){
 ...
 $rate=round(($assprofit*100)).'%';
}
if($taxtype=="EDT"){
 $rate=round(getSettings2($serial,'edtrate',$conn),1).'%';
}

if($taxtype=="WHT" && $whttnx=="yes" || $taxtype=="CGT" && $cgttnx=="yes"){
	...
	$assprofit=0.00;
}elseif ($taxtype=="WHT" && $whttnx=='no'|| $taxtype=="CGT" && $cgttnx=='no') {
	$amount=$tprofit;
	$amtdue=$amount;
...
if($taxtype=="CIT" && $display=="true" ){
	...
	$amount=$tprofit*(getSettings2($serial,'citrate',$conn)/100);
	$amtdue=$amount-$amtpaid;
	$penalty=checkpen($penalty,$amtdue);
...
if($taxtype=="EDT" && $display=="true"){
	$tprofit=0.00;
	$amount=$assprofit*(getSettings2($serial,'edtrate',$conn)/100);
	$amtdue=$amount-$amtpaid;
	$penalty=checkpen($penalty,$amtdue);
...
if($taxtype=="TP"){
    $basis="Transfer Pricing Late Filing";
    }
if($taxtype=="NITDL"){
		$basis="NITDL Levy";
	}
...
if($taxtype=='WHT' && $tprofit==$amount || $taxtype=='CGT' && $tprofit==$amount){
	$rate="";
}
    ...
    if($taxtype=="CGT") {
      ...
      $bgpic= "img/bgcgt.jpg";
		...
		if($assprofit==0.00){
      		...
      		$showassp='';
        }else if ($taxtype=="WHT" && $assprofit>0 || $taxtype=="CGT" && $assprofit>0) {
      					$showassp='';
      					  }else{
      			$showassp= number_format($assprofit,2) ;
		  }
		  if($tprofit==0.00){
				  	...
				  	$showtp	='';
          }else if ($taxtype=="WHT" && $tprofit>0 || $taxtype=="CGT" && $tprofit>0) {
				  		$showtp	='';
				  	}else {
